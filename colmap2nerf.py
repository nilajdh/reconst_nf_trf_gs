...
if __name__ == "__main__":
	...
	# Sort frames by filename to ensure consistent order
	...
	# Split into train/test sets (e.g., last N images as test)
	NUM_TEST_IMAGES = 10  # 自定义测试图像数量
	test_frames = out["frames"][-NUM_TEST_IMAGES:]
	train_frames = out["frames"][:-NUM_TEST_IMAGES]
	...
	with open("transforms_train.json", "w") as train_file:
		json.dump(train_out, train_file, indent=2)

	with open("transforms_test.json", "w") as test_file:
		json.dump(test_out, test_file, indent=2)

	print(f"Wrote {len(train_frames)} train and {len(test_frames)} test frames")
